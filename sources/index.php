...
$slider = $d->rawQuery("select ten$lang as ten, photo, mota$lang as mota, noidung$lang as noidung, link, mau from #_photo where type = ? and hienthi > 0 order by stt,id desc",array('slide'));
...
$gioithieu = $d->rawQueryOne("select id, type, ten$lang as ten, mota$lang as mota, photo, ngaytao, ngaysua from #_static where type = ? limit 0,1",array('gioi-thieu'));
    ...
    // $brand = $d->rawQuery("select ten$lang as ten, $sluglang, id, photo from #_product_brand where type = ? and hienthi > 0 order by stt,id desc",array('san-pham'));
$danhmuc1sanphamnb = $d->rawQuery("select ten$lang as ten, $sluglang, id, noibat from #_product_list where type = ? and noibat > 0 and hienthi > 0 order by stt,id desc",array('san-pham'));
$sanphamnb = $d->rawQuery("select id from #_product where type = ? and noibat > 0 and hienthi > 0",array('san-pham'));

$tintucnb = $d->rawQuery("select ten$lang as ten, $sluglang, mota$lang as mota, ngaytao, id, photo from #_news where type = ? and noibat > 0 and hienthi > 0 order by stt,id desc",array('tin-tuc'));

$thuvienanh = $d->rawQuery("select ten$lang as ten, $sluglang, mota$lang as mota, ngaytao, id, photo from #_product where type = ? and noibat > 0 and hienthi > 0 order by stt,id desc limit 0,5",array('thu-vien-anh'));
...
$tagsnhansu = $d->rawQuery("select ten$lang as ten, id from #_tags where hienthi > 0 and type = ? order by stt, id desc", array("member"));
