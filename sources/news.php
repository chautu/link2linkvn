<?php  
	...
	$menu_news = $d->rawQuery("select id, ten$lang as ten, $sluglang from #_news_list where type = ? and hienthi > 0 order by stt, id desc", array($type));

	$tinnb = $d->rawQuery("select id, ten$lang as ten, $sluglang, mota$lang, photo from #_news where type = ? and hienthi > 0 and noibat > 0 order by stt, id desc", array($type));

	if($id!='')
	{
		/* Lấy bài viết detail */
		$row_detail = $d->rawQueryOne("select id, luotxem, ngaytao, id_list, id_cat, id_item, id_sub, type, ten$lang as ten, $sluglang, noidung$lang as noidung, photo from #_news where id = ? and type = ? and hienthi > 0 limit 0,1",array($id,$type));
		...
		/* Lấy cấp 1 */
		$news_list = $d->rawQueryOne("select id, ten$lang as ten, $sluglang from #_news_list where id = ? and type = ? and hienthi > 0 limit 0,1",array($row_detail['id_list'],$type));

		/* Lấy cấp 2 */
		$news_cat = $d->rawQueryOne("select id, ten$lang as ten, $sluglang from #_news_cat where id = ? and type = ? and hienthi > 0 limit 0,1",array($row_detail['id_cat'],$type));

		/* Lấy cấp 3 */
		$news_item = $d->rawQueryOne("select id, ten$lang as ten, $sluglang from #_news_item where id = ? and type = ? and hienthi > 0 limit 0,1",array($row_detail['id_item'],$type));

		/* Lấy cấp 4 */
		$news_sub = $d->rawQueryOne("select id, ten$lang as ten, $sluglang from #_news_sub where id = ? and type = ? and hienthi > 0 limit 0,1",array($row_detail['id_sub'],$type));	
		...
		$sql = "select id, ten$lang as ten, $sluglang, photo, ngaytao, mota$lang as mota from #_news where $where order by stt,id desc $limit";
		...
		if(!empty($news_list) && $news_list['ten']) $breadcr->setBreadCrumbs($news_list[$sluglang],$news_list['ten']);
		if(!empty($news_cat) && $news_cat['ten']) $breadcr->setBreadCrumbs($news_cat[$sluglang],$news_cat['ten']);
		if(!empty($news_item) && $news_item['ten']) $breadcr->setBreadCrumbs($news_item[$sluglang],$news_item['ten']);
		if(!empty($news_sub) && $news_sub['ten']) $breadcr->setBreadCrumbs($news_sub[$sluglang],$news_sub['ten']);
		if(!empty($row_detail) && $row_detail['ten']) $breadcr->setBreadCrumbs($row_detail[$sluglang],$row_detail['ten']);
		$breadcrumbs = $breadcr->getBreadCrumbs();
	}
	else if($idl!='')
	{
		/* Lấy cấp 1 detail */
		$news_list = $d->rawQueryOne("select id, ten$lang as ten, $sluglang, type, photo from #_news_list where id = ? and type = ? limit 0,1",array($idl,$type));
		...
		if(isset($_GET['keyword-news']) && $_GET['keyword-news']) {
			$tukhoas = htmlspecialchars($_GET['keyword-news']);
	...
	{
		/* Lấy cấp 2 detail */
		$news_cat = $d->rawQueryOne("select id, id_list, ten$lang as ten, $sluglang, type, photo from #_news_cat where id = ? and type = ? limit 0,1",array($idc,$type));

		/* Lấy cấp 1 */
		$news_list = $d->rawQueryOne("select id, ten$lang as ten, $sluglang from #_news_list where id = ? and type = ? limit 0,1",array($news_cat['id_list'],$type));
	...
	{
		/* Lấy cấp 3 detail */
		$news_item = $d->rawQueryOne("select id, id_list, id_cat, ten$lang as ten, $sluglang, type, photo from #_news_item where id = ? and type = ? limit 0,1",array($idi,$type));

		/* Lấy cấp 1 */
		$news_list = $d->rawQueryOne("select id, ten$lang as ten, $sluglang from #_news_list where id = ? and type = ? limit 0,1",array($news_item['id_list'],$type));

		/* Lấy cấp 2 */
		$news_cat = $d->rawQueryOne("select id, ten$lang as ten, $sluglang from #_news_cat where id_list = ? and id = ? and type = ? limit 0,1",array($news_item['id_list'],$news_item['id_cat'],$type));
	...
	{
		/* Lấy cấp 4 */
		$news_sub = $d->rawQueryOne("select id, id_list, id_cat, id_item, ten$lang as ten, $sluglang, type, photo from #_news_sub where id = ? and type = ? limit 0,1",array($ids,$type));

		/* Lấy cấp 1 */
		$news_list = $d->rawQueryOne("select id, ten$lang as ten, $sluglang from #_news_list where id = ? and type = ? limit 0,1",array($news_sub['id_list'],$type));

		/* Lấy cấp 2 */
		$news_cat = $d->rawQueryOne("select id, ten$lang as ten, $sluglang from #_news_cat where id_list = ? and id = ? and type = ? limit 0,1",array($news_sub['id_list'],$news_sub['id_cat'],$type));

		/* Lấy cấp 3 */
		$news_item = $d->rawQueryOne("select id, ten$lang as ten, $sluglang from #_news_item where id_list = ? and id_cat = ? and id = ? and type = ? limit 0,1",array($news_sub['id_list'],$news_sub['id_cat'],$news_sub['id_item'],$type));
