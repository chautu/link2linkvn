...
'@<[\ /\!]*?[^<>]*?>@si', // Loại bỏ HTML tags
	...
	public function get_payments($id=0,$lang="vi")
	{
		if($id)
		{
			$row = $this->d->rawQueryOne("select ten$lang as ten from #_news where id = ? limit 0,1",array($id));
			return $row['ten'];
		}
		else
		{
	...
	/* Lấy places */
	public function get_places($table='', $id=0, $lang="vi")
	{
		if($table && $id)
		{
			$row = $this->d->rawQueryOne("select ten$lang as ten from #_$table where id = ? limit 0,1",array($id));
			return $row['ten'];
		}
		else
		{
	...
	/* Lấy status */
	public function get_namestatus($table='', $id=0, $default="Mới tạo" , $lang="vi")
	{
		if($table && $id)
		{
			$row = $this->d->rawQueryOne("select ten$lang as ten, span, id from #_$table where id = ? limit 0,1",array($id));
			return $row['ten'];
		}
		else
		{
	...
	public function get_status_by_default($table="", $col="id", $where="access", $type="transition", $lang="vi")
	{
		$row = $this->d->rawQueryOne("select $col from #_$table where $where = 1 and type = ? limit 0,1", array($type));
		return $row[$col] ;
	}

	/* Lấy status */
	public function get_status($table='', $id=0, $default="Mới tạo" , $lang="vi")
	{
		if($table && $id)
		{
			$row = $this->d->rawQueryOne("select ten$lang as ten, span, id from #_$table where id = ? limit 0,1",array($id));
			return "<span class='".$row['span']."'>".$row['ten']."</span>" ;
		}
		else
		{
			return "<span class='text-primary'>".$default."</span>" ;
		}
	}
	...
	public function checkLogin()
	{
		...
		if(isset($_SESSION[$login_member]) || isset($_COOKIE['login_member_id']))
		{
			...
			if($iduser)
			{
				$row = $this->d->rawQueryOne("select login_session, id, username, dienthoai, email, ten from #_member where id = ? and hienthi > 0",array($iduser));
	...
	/* Transfer */
	public function transferUser($msg='', $page='', $stt=true)
	{
		global $config_base;

		$_SESSION['flash_messager'] = $msg;
		$_SESSION['status_messager'] = $stt;
	...
	public function uploadImage($file='', $extension='', $folder='', $newname='')
	{
		...
		public function createThumb($width_thumb=0, $height_thumb=0, $zoom_crop='1', $src="", $watermark=null, $path=THUMBS, $preview=false, $args=array(), $quality=100)
		{
			...
			if($image_ext != 'svg' && $image_ext != 'webp') {
			...
			switch(strtoupper($image_ext))
			{
				...
				$svgContent = file_get_contents($image_url);
				echo $svgContent;
				die();

				case 'WEBP':
				header('Content-Type: image/webp');
				$svgContent = readfile($image_url);
				echo $svgContent;
				die();
		...
		/* Get permission */
		public function get_permission($id_permission=0, $required=false)
		{
			$row = $this->d->rawQuery("select * from #_permission_group where hienthi>0 order by stt,id desc");

			$str = '<select id="id_nhomquyen" name="data[id_nhomquyen]" class="form-control select2" '.($required ? 'required' : '').'>
			<option value="" disabled="" selected="" hidden="">Nhóm quyền</option>';
			foreach($row as $v)
			{
		...
		/* Get status order */
		public function orderStatus($status=0, $lang="vi", $required=false)
		{
			$row = $this->d->rawQuery("select ten$lang as ten, id, span from #_status where type=? order by stt, id desc", array('order'));

			$str = '<select id="tinhtrang" name="data[tinhtrang]" class="form-control text-sm select2" '.($required ? 'required' : '').'>
		...
		/* Get payments order */
		function orderPayments($lang="vi", $required=false)
		{
			$row = $this->d->rawQuery("select id, ten$lang as ten from #_news where type='hinh-thuc-thanh-toan' order by stt,id desc");

			$str = '<select id="httt" name="httt" class="form-control text-sm select2" '.($required ? 'required' : '').'>
		...
		/* Get tags */
		public function get_tags($id=0, $element='', $table='', $type='', $lang="vi")
		{
			if($id)
			{
		...
		$row_tags = $this->d->rawQuery("select ten$lang as ten, id from #_tags where type = ? order by stt,id desc",array($type));
		...
		for($i=0;$i<count($row_tags);$i++)
		{
			...
			$str .= '<option value="'.$row_tags[$i]["id"].'" '.$selected.' /> '.$row_tags[$i]['ten'].'</option>';
		}
		$str .= '</select>';
	...
	public function get_ajax_category($table='', $level='', $type='', $lang="vi", $required=false, $title_select='Chọn danh mục', $class_select='select-category')
	{
		$where = '';
		$params = array($type);
		$id_parent = 'id_'.$level;
		...
		$rows = $this->d->rawQuery("select ten$lang as ten, id from #_".$table."_".$level." where type = ? ".$where." order by stt,id desc",$params);

		$str = '<select id="'.$id_parent.'" name="data['.$id_parent.']" '.$data_level.' '.$data_type.' '.$data_table.' '.$data_child.' class="form-control select2 '.$class_select.'" '.($required ? 'required' : '').'>
		<option value="" disabled="" selected="" hidden="">'.$title_select.'</option>';
		foreach($rows as $v)
		{
	...
	public function get_link_category($table='', $level='', $type='', $lang="vi",$required=false, $title_select='Chọn danh mục')
	{
		global $linkFilter;
		...
		$str = '<select id="'.$id_parent.'" name="'.$id_parent.'" onchange="onchange_category($(this), '."'".$linkFilter."'".')" class="form-control filer-category select2" '.($required ? 'required' : '').'>
					<option value="" selected="" hidden="">'.$title_select.'</option>';
		foreach($rows as $v)
		{
			if(isset($_REQUEST[$id_parent]) && ($v["id"] == (int)$_REQUEST[$id_parent])) $selected = "selected";
			else $selected = "";
	...
	/* Get place by ajax */
	public function get_ajax_place($table='',$lang="vi", $required=false, $title_select='Chọn danh mục')
	{
		$where = '';
		$params = array('0');
		$id_parent = 'id_'.$table;
		...
		else if($table == "street") 
		{
			...
			$idwards = (isset($_REQUEST['id_wards'])) ? htmlspecialchars($_REQUEST['id_wards']) : 0;
			$where .= ' and id_wards = ?';
			array_push($params, $idwards);
		}

		$rows = $this->d->rawQuery("select ten$lang as ten, id from #_".$table." where id <> ? ".$where." order by id asc",$params);

		$str = '<select id="'.$id_parent.'" name="data['.$id_parent.']" '.$data_level.' '.$data_table.' '.$data_child.' class="form-control select2 select-place" '.($required ? 'required' : '').'>
					<option value="" disabled="" selected="" hidden="">'.$title_select.'</option>';
		foreach($rows as $v)
		{
			if(isset($_REQUEST[$id_parent]) && ($v["id"] == (int)$_REQUEST[$id_parent])) $selected = "selected";
	...
	/* Get place by link */
	public function get_link_place($table='',$lang= "vi", $required=false, $title_select='Chọn danh mục')
	{
		global $linkFilter;
	...
	/* Get search */
	public function get_search($table='', $type='', $lang='vi', $required=false , $title_select='Chọn hướng', $class_select='select-category')
	{
		$id_parent = 'id_'.$type.$lang;

		$rows = $this->d->rawQuery("select ten$lang as ten, id from #_".$table." where hienthi = 1 and type = '".$type."' order by stt asc,id desc");


		$str = '<select id="'.$id_parent.'" name="data['.$id_parent.']" class="form-control select2 '.$class_select.'" '.($required?'required':'').'>
					<option value="" disabled="" selected="" hidden="">'.$title_select.'</option>';
		foreach($rows as $v)
		{
			if(isset($_REQUEST[$id_parent]) && ($v["id"] == (int)$_REQUEST[$id_parent])) $selected = "selected";
	...
	/* Get Bank */
	public function get_bank($table="", $type="", $lang ="vi", $required=false, $title_select="Chọn ngân hàng", $class_select='select-banking')
	{
		$id_parent = 'id_'.$type;

		$rows = $this->d->rawQuery("select ten$lang as ten, id from #_".$table." where hienthi = 1 and type = '".$type."' order by stt,id desc");
	...
	/* Get Status */
	public function get_select_status($table="", $type="", $lang ="vi", $required=false, $title_select="Chọn trạng thái", $class_select='select-status')
	{
		$id_parent = 'id_'.$table;

		$rows = $this->d->rawQuery("select ten$lang as ten, id from #_".$table." where type = '".$type."' order by stt,id desc");
	...
	/* Get link status */
	public function get_link_status($table="", $type="", $lang ="vi", $required=false, $title_select="Lọc trạng thái", $class_select='filer-category')
	{
		global $linkFilter;
		$id_parent = 'id_'.$table;
		...
		$str = '<select id="'.$id_parent.'" name="'.$id_parent.'" onchange="onchange_category($(this), '."'".$linkFilter."'".')" class="form-control select2 '.$class_select.'" '.($required?'required':'').'>
					<option value="" selected="" hidden="">'.$title_select.'</option>';
		foreach($rows as $v)
		{
			if(isset($_REQUEST[$id_parent]) && ($v["id"] == (int)$_REQUEST[$id_parent])) $selected = "selected";
	...
	/* Get banking transition */
	public function get_banknap($lang="vi",$required=false)
	{
		$rows = $this->d->rawQuery("select ten$lang as ten, id from #_news where type='bank' and hienthi > 0 order by stt,id desc");
		$str = '<select id="id_banknap" name="id_banknap" class="form-control select2" '.($required?'required':'').'>
					<option value="" disabled="" selected="" hidden="">Chọn ngân hàng nạp</option>';
		foreach($rows as $v)
		{
			if(isset($_REQUEST['id_banknap']) && ($v["id"] == (int)$_REQUEST['id_banknap'])) $selected = "selected";
			else $selected = "";
	...
	/* Get banking transition */
	public function get_bankrut($id,$lang="vi",$required=false)
	{
		$rows = $this->d->rawQuery("select id, chutk, stk, id_bank from #_user_bank where id_user=? and hienthi > 0 order by stt,id desc", array($id));
		$str = '<select id="id_bankrut" name="id_bankrut" class="form-control select2" '.($required?'required':'').'>
					<option value="" disabled="" selected="" hidden="">Chọn ngân hàng rút</option>';
		foreach($rows as $v)
		{
			if(isset($_REQUEST['id_bankrut']) && ($v["id"] == (int)$_REQUEST['id_bankrut'])) $selected = "selected";
			else $selected = "";

			$bank_name = $this->get_places("news_list", $v['id_bank']);

			$str .= '<option value='.$v["id"].' '.$selected.'>'.$bank_name.' - '.$v["stk"].'</option>';
		}
		$str .= '</select>';
	...
	/* Get rows table by col */
	public function get_rows($table="",$col="", $id=0) 
	{
		$row = null;
		if($id && $table && $col) {
			$row = $this->d->rawQuery("select * from #_$table where $col = ? order by stt, id desc", array($id));
		}
		return $row;
	}

	/* Get member */
	public function get_profile($id = 0, $table="") {
		$row = null;
		if($id && $table) {
			$row = $this->d->rawQueryOne("select * from #_$table where id = ? order by stt, id desc", array($id));
		}
		return $row;
	}

	function get_row($table="", $wherecol="", $vlcol=0, $col="")
	{
		$row = null;
		if($table) {
			$row = $this->d->rawQueryOne("select $col from #_$table where $wherecol = ? order by stt, id desc", array($vlcol));
		}
		return $row[$col];
	}
	...
	function payment_vnpay($id, $money, $bankCode, $url_return)
	{
		global $config;
		...
		$vnp_Locale = $config['website']['lang-default'];
		...
		if (isset($config['vnpay']['vnp_HashSecret'])) {
	...
	function check_transition_vnpay($vnp_SecureHash, $money, $id, $responeCode, $table)
	{
		global $config;
		...
		{
			$transition = $this->d->rawQueryOne("select * from #_$table where id = ?", array($id));
			if($responeCode == '00') 
			{
				if ($money != $transition['money']*100) {
					return 01;
				} else {
					return 02;
				}
	...
	function getPhotoTimeline($id = 0)
	{
		global $d;
		if($id) {
			$row = $d->rawQuery("select photo, link_video, taptin, type, mota from #_member_photo where id_timeline = ?", array($id));
			return $row;
		}
	}
