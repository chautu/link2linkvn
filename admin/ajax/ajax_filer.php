<?php
	...
	if($cmd == 'refresh' && ($idmuc > 0 || $hash != ''))
	{
		if($idmuc)
		{
			$gallery = $d->rawQuery("select stt, id, photo, ten".$config['website']['lang-default']." from #_gallery where id_photo = ? and com = ? and type = ? and kind = ? and val = ? order by stt,id desc",array($idmuc,$com,$type,$kind,$type));
		}
		else if($hash != '')
		{
			$gallery = $d->rawQuery("select stt, id, photo, ten".$config['website']['lang-default']." from #_gallery where hash = ? and com = ? and type = ? and kind = ? and val = ? order by stt,id desc",array($hash,$com,$type,$kind,$type));
		}
		...
		if($gallery)
		{
			...
			{
				echo $func->galleryFiler($gallery[$i]['stt'],$gallery[$i]['id'],$gallery[$i]['photo'],$gallery[$i]['ten'.$config['website']['lang-default']],$com,$colfiler);
			}
	...
	else if($cmd == 'info' && $id > 0 && ($idmuc > 0 || $hash != ''))
	{
		...
		if($info == 'tieude')
		{
			$data['ten'.$config['website']['lang-default']] = $value;
		}
