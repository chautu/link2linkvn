<?php	
	...
	function exportExcel()
	{
		...
		if(isset($_POST['exportExcel']))
		{
			...
			$PHPExcel->getProperties()->setCreator($setting['ten'.$config['website']['lang-default']]);
			$PHPExcel->getProperties()->setLastModifiedBy($setting['ten'.$config['website']['lang-default']]);
			...
			$array_file = array(
				...
				'id_cat'=>'Danh mục 2',
				'masp'=>'Mã sản phẩm',
				'ten'.$config['website']['lang-default'] =>'Tên Sản phẩm',
				'gia'=>'Giá bán',
				'giamoi'=>'Giá mới',
				'giakm'=>'Chiết khấu',
				'mota'.$config['website']['lang-default']=>'Mô tả',
				'noidung'.$config['website']['lang-default']=>'Nội dung',
				'photo'=>'Hình đại diện'
			);
			...
			foreach($array_file as $k=>$v)
			{
				...
				else if($k=='ten'.$config['website']['lang-default'])
				{
				...
				else if($k=='mota'.$config['website']['lang-default'] || $k=='noidung'.$config['website']['lang-default'] || $k=='photo')
				{
			...
			for($i=0;$i<count($product);$i++)
			{
				...
				foreach($array_file as $k=>$v)
				{
					if($k=='id_list')
					{
						$tenlist = $d->rawQueryOne("select ten".$config['website']['lang-default']." from #_product_list where id = ? limit 0,1",array($product[$i][$k]));
						$datacell = $tenlist['ten'.$config['website']['lang-default']];
					}
					else if($k=='id_cat')
					{
						$tencat = $d->rawQueryOne("select ten".$config['website']['lang-default']." from #_product_cat where id = ? limit 0,1",array($product[$i][$k]));
						$datacell = $tencat['ten'.$config['website']['lang-default']];
					}
					else
					{
