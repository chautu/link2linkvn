				...
				foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) 
				{
					...
					for($row=2;$row<=$highestRow;++$row)
					{
						...
						if($masp!="")
						{
							...
							$idlist = $d->rawQueryOne("select id from #_product_list where tenkhongdau".$config['website']['lang-default']." = ? limit 0,1",array($tkdcap1));

							/* Lấy danh mục cấp 2 */
							$tkdcap2 = $func->changeTitle($cap2);
							$idcat = $d->rawQueryOne("select id from #_product_cat where tenkhongdau".$config['website']['lang-default']." = ? limit 0,1",array($tkdcap2));
							...
							$data['ten'.$config['website']['lang-default']] = ($ten != '') ? htmlspecialchars($ten) : '';
							$data['tenkhongdau'.$config['website']['lang-default']] = ($data['ten'.$config['website']['lang-default']] != '') ? $func->changeTitle($data['ten'.$config['website']['lang-default']]) : '';
							$data['gia'] = $gia;
							$data['giamoi'] = $giamoi;
							$data['giakm'] = $giakm;
							$data['mota'.$config['website']['lang-default']] = ($mota != '') ? htmlspecialchars($mota) : '';
							$data['noidung'.$config['website']['lang-default']] = ($noidung != '') ? htmlspecialchars($noidung) : '';
