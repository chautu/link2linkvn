...
switch($act)
{
	...
	case "man_attr":
	get_attrs();
	$template = "product/attr/attrs";
	break;
	case "add_attr":
	$template = "product/attr/attr_add";
	break;
	case "edit_attr":
	get_attr();
	$template = "product/attr/attr_add";
	...
	case "man_attributes":
	get_attributes();
	$template = "product/attributes/attributes";
	break;
	case "add_attributes":
	$template = "product/attributes/attributes_add";
	break;
...
function get_items()
{
	global $d, $func, $strUrl, $curPage, $items, $paging, $type, $config;
...
function get_item()
{
	...
	if($act != 'copy')
	{
		$gallery = $d->rawQuery("select * from #_gallery where id_photo = ? and type_hienthi = 0 and com = ? and type = ? and kind = ? and val = ? order by stt,id desc",array($id,$com,$type,'man',$type));
	}
}
...
function save_item()
{
	...
	if($data)
	{
		...
		if(isset($config['product'][$type]['attr']) && $config['product'][$type]['attr'] == true)
		{
			if(isset($_POST['attr_group']) && ($_POST['attr_group'] != '')) $data['id_attr'] = implode(",", $_POST['attr_group']);
			else $data['id_attr'] = "";
		}
...
function get_attrs()
{
	...
	$startpoint = ($curPage * $per_page) - $per_page;
	$limit = " limit ".$startpoint.",".$per_page;
	$sql = "select * from #_product_attr where type = ? $where order by stt,id desc $limit";
	$items = $d->rawQuery($sql,array($type));
	$sqlNum = "select count(*) as 'num' from #_product_attr where type = ? $where order by stt,id desc";
	$count = $d->rawQueryOne($sqlNum,array($type));
	$total = $count['num'];
	$url = "index.php?com=product&act=man_attr&type=".$type;
	$paging = $func->pagination($total,$per_page,$curPage,$url);
}
...
function get_attr()
{
	...
	if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_attr&type=".$type."&p=".$curPage.$strUrl, false);

	$item = $d->rawQueryOne("select * from #_product_attr where id = ? and type = ? limit 0,1",array($id,$type));
	
	if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=product&act=man_attr&type=".$type."&p=".$curPage.$strUrl, false);

	/* Lấy hình ảnh con */
	$gallery = $d->rawQuery("select * from #_gallery where id_photo = ? and com = ? and type = ? and kind = ? and val = ? order by stt,id desc",array($id,$com,$type,'man_attr',$type));
}
...
function save_attr()
{
	...
	if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_attr&type=".$type."&p=".$curPage.$strUrl, false);
	...
	if($id)
	{
		...
		{
			$file_name = $func->uploadName($_FILES['file']["name"]);
			if($photo = $func->uploadImage("file", $config['product'][$type]['img_type_attr'], UPLOAD_ATTR,$file_name))
			{
				$data['photo'] = $photo;
				$row = $d->rawQueryOne("select id, photo from #_product_attr where id = ? and type = ? limit 0,1",array($id,$type));
				if(isset($row['id']) && $row['id'] > 0) $func->delete_file(UPLOAD_ATTR.$row['photo']);
			}
		}
		...
		$d->where('id', $id);
		$d->where('type', $type);
		if($d->update('product_attr',$data))
		{
			$func->transfer("Cập nhật dữ liệu thành công", "index.php?com=product&act=man_attr&type=".$type."&p=".$curPage.$strUrl);
		}
		else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_attr&type=".$type."&p=".$curPage.$strUrl, false);
	}
	else
	{
		if(isset($_FILES['file']))
		...
		if($d->insert('product_attr',$data))
		{
			$id_insert = $d->getLastInsertId();

			$func->transfer("Lưu dữ liệu thành công", "index.php?com=product&act=man_attr&type=".$type."&p=".$curPage.$strUrl);
		}
		else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_attr&type=".$type."&p=".$curPage.$strUrl, false);
	}
}
...
function delete_attr()
{
	...
	if($id)
	{
		...
		if(isset($row['id']) && $row['id'] > 0)
		{
			$func->delete_file(UPLOAD_ATTR.$row['photo']);
			$d->rawQuery("delete from #_product_attr where id = ?",array($id));
			$func->transfer("Xóa dữ liệu thành công", "index.php?com=product&act=man_attr&type=".$type."&p=".$curPage.$strUrl);
		}
		else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man_attr&type=".$type."&p=".$curPage.$strUrl, false);
	}
	elseif(isset($_GET['listid']))
	{
		$listid = explode(",",$_GET['listid']);
	...
	else $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_attr&type=".$type."&p=".$curPage.$strUrl, false);
}
...
function get_attributes()
{
	...
	$idattr = (isset($_REQUEST['id_attr'])) ? htmlspecialchars($_REQUEST['id_attr']) : 0;

	if($idattr) $where .= " and id_attr=$idattr";
	...
	$startpoint = ($curPage * $per_page) - $per_page;
	$limit = " limit ".$startpoint.",".$per_page;
	$sql = "select * from #_product_attributes where type = ? $where order by stt,id desc $limit";
	$items = $d->rawQuery($sql,array($type));
	$sqlNum = "select count(*) as 'num' from #_product_attributes where type = ? $where order by stt,id desc";
	$count = $d->rawQueryOne($sqlNum,array($type));
	$total = $count['num'];
	$url = "index.php?com=product&act=man_attributes&type=".$type;
	$paging = $func->pagination($total,$per_page,$curPage,$url);
}
...
function get_attribute()
{
	...
	if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_attributes&type=".$type."&p=".$curPage.$strUrl, false);

	$item = $d->rawQueryOne("select * from #_product_attributes where id = ? and type = ? limit 0,1",array($id,$type));
	
	if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=product&act=man_attributes&type=".$type."&p=".$curPage.$strUrl, false);
	
	/* Lấy hình ảnh con */
	$gallery = $d->rawQuery("select * from #_gallery where id_photo = ? and com = ? and type = ? and kind = ? and val = ? order by stt,id desc",array($id,$com,$type,'man_attributes',$type));
}
...
function save_attributes()
{
	...
	if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_attributes&type=".$type."&p=".$curPage.$strUrl, false);
	...
	if($id)
	{
		...
		{
			$file_name = $func->uploadName($_FILES['file']["name"]);
			if($photo = $func->uploadImage("file", $config['product'][$type]['img_type_attributes'], UPLOAD_ATTR,$file_name))
			{
				$data['photo'] = $photo;
				$row = $d->rawQueryOne("select id, photo from #_product_attributes where id = ? and type = ? limit 0,1",array($id,$type));
				if(isset($row['id']) && $row['id'] > 0) $func->delete_file(UPLOAD_ATTR.$row['photo']);
			}
		}
		...
		$d->where('id', $id);
		$d->where('type', $type);
		if($d->update('product_attributes',$data))
		{
			$func->transfer("Cập nhật dữ liệu thành công", "index.php?com=product&act=man_attributes&type=".$type."&p=".$curPage.$strUrl);
		}
		else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_attributes&type=".$type."&p=".$curPage.$strUrl, false);
	}
	else
	{
		if(isset($_FILES['file']))
		...
		if($d->insert('product_attributes',$data))
		{
			$id_insert = $d->getLastInsertId();

			$func->transfer("Lưu dữ liệu thành công", "index.php?com=product&act=man_attributes&type=".$type."&p=".$curPage.$strUrl);
		}
		else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_attributes&type=".$type."&p=".$curPage.$strUrl, false);
	}
}
...
function delete_attributes()
{
	...
	if($id)
	{
		...
		{
			$func->delete_file(UPLOAD_ATTR.$row['photo']);
			$d->rawQuery("delete from #_product_attributes where id = ?",array($id));
			$func->transfer("Xóa dữ liệu thành công", "index.php?com=product&act=man_attributes&type=".$type."&p=".$curPage.$strUrl);
		}
		else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man_attributes&type=".$type."&p=".$curPage.$strUrl, false);
	}
	elseif(isset($_GET['listid']))
	{
		$listid = explode(",",$_GET['listid']);
	...
	else $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_attributes&type=".$type."&p=".$curPage.$strUrl, false);
}
