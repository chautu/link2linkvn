<?php
	...
	function get_items()
	{
		...
		$orderMoidat = $d->rawQueryOne("select count(id), sum(tonggia) from #_order where tinhtrang = 0");
		$allMoidat = $orderMoidat['count(id)'];
		$totalMoidat = $orderMoidat['sum(tonggia)'];

		/* Lấy đơn hàng - đã xác nhận */
		$orderDaxacnhan = $d->rawQueryOne("select count(id), sum(tonggia) from #_order where tinhtrang = ?", array($func->get_status_by_default("status", 'id', 'await', 'order')));
		$allDaxacnhan = $orderDaxacnhan['count(id)'];
		$totalDaxacnhan = $orderDaxacnhan['sum(tonggia)'];

		/* Lấy đơn hàng - đã giao */
		$orderDagiao = $d->rawQueryOne("select count(id), sum(tonggia) from #_order where tinhtrang = ?",  array($func->get_status_by_default("status", 'id', 'access', 'order')));
		$allDagiao = $orderDagiao['count(id)'];
		$totalDagiao = $orderDagiao['sum(tonggia)'];

		/* Lấy đơn hàng - đã hủy */
		$orderDahuy = $d->rawQueryOne("select count(id), sum(tonggia) from #_order where tinhtrang = ?",  array($func->get_status_by_default("status", 'id', 'cancel', 'order')));
