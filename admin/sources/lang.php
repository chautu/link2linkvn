	...
	function get_items()
	{
		...
		if(isset($_REQUEST['keyword']))
		{
			...
			foreach($config['website']['lang'] as $lang => $key) {
				if($index > 0) { $where .= " or "; };
				$where .= "lang$lang LIKE '%$keyword%'";
				$index += 1;
			}
			$where .= ")";
			$where .= " or giatri LIKE '%$keyword%' ";
		}

		$per_page = 10;
		$startpoint = ($curPage * $per_page) - $per_page;
		$limit = " limit ".$startpoint.",".$per_page;
		$sql = "select * from #_lang where id <> 0 $where order by id desc $limit";
		$items = $d->rawQuery($sql);
		$sqlNum = "select count(*) as 'num' from #_lang where id <> 0 $where order by id desc";
