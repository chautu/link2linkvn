                                ...
                                <div class="tab-content" id="custom-tabs-three-tabContent-lang">
                                    ...
                                    <div class="tab-pane fade show <?=($k==$config['website']['lang-default'])?'active':''?>" id="tabs-lang-<?=$k?>"
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea
                                                class="form-control for-seo short <?=(isset($config['news'][$type]['mota_cke']) && $config['news'][$type]['mota_cke'] == true)?'form-control-ckeditor':''?>"
                                                name="data[mota<?=$k?>]" id="mota<?=$k?>" rows="5"
                                                placeholder="Mô tả (<?=$k?>)"><?=html_entity_decode(@$item['mota'.$k])?></textarea>
                                        </div>
                                        <?php } ?>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea
                                                ...
                                                placeholder="Nội dung (<?=$k?>)"><?=(@$item['noidung'.$k])?></textarea>
                                        </div>
                                        <?php } ?>
                                    </div>
                                    <?php } ?>
                                </div>
                                <?php if(isset($config['news'][$type]['stk']) && $config['news'][$type]['stk'] == true) { ?>
                                    <div class="form-group">
                                        <label for="stk">Số tài khoản:</label>
                                        <input type="text" class="form-control for-seo" name="data[stk]"
                                            id="stk" placeholder="Số tài khoản"
                                            value="<?=@$item['stk']?>" required>
                                    </div>
                                <?php } ?>
                                <?php if(isset($config['news'][$type]['chutk']) && $config['news'][$type]['chutk'] == true) { ?>
                                    <div class="form-group">
                                        <label for="chutk">Chủ tài khoản:</label>
                                        <input type="text" class="form-control for-seo" name="data[chutk]"
                                            id="chutk" placeholder="Chủ tài khoản"
                                            value="<?=@$item['chutk']?>" required>
                                    </div>
                                <?php } ?>
                            </div>
                        </div>
                    ...
                    <div class="card-body">
                        <div class="form-group-category row">
                            ...
                            <div class="form-group col-xl-6 col-sm-4">
                                ...
                                <?=$func->get_ajax_category('news', 'list', $type, $config['website']['lang-default'])?>
                            </div>
                            <?php } ?>
                            <?php if(isset($config['news'][$type]['cat']) && $config['news'][$type]['cat'] == true) { ?>
                            <div class="form-group col-xl-6 col-sm-4">
                                <label class="d-block" for="id_cat"><?= $config['news'][$type]['title_main_cat'] ?>:</label>
                                <?=$func->get_ajax_category('news', 'cat', $type, $config['website']['lang-default'])?>
                            </div>
                            <?php } ?>
                            <?php if(isset($config['news'][$type]['item']) && $config['news'][$type]['item'] == true) { ?>
                            <div class="form-group col-xl-6 col-sm-4">
                                <label class="d-block" for="id_item"><?= $config['news'][$type]['title_main_item'] ?>:</label>
                                <?=$func->get_ajax_category('news', 'item', $type, $config['website']['lang-default'])?>
                            </div>
                            <?php } ?>
                            <?php if(isset($config['news'][$type]['sub']) && $config['news'][$type]['sub'] == true) { ?>
                            <div class="form-group col-xl-6 col-sm-4">
                                <label class="d-block" for="id_sub"><?= $config['news'][$type]['title_main_sub'] ?>:</label>
                                <?=$func->get_ajax_category('news', 'sub', $type, $config['website']['lang-default'])?>
                            </div>
                            <?php } ?>
                            <?php } ?>
                            ...
                            <div class="form-group col-xl-6 col-sm-4">
                                ...
                                <?=$func->get_tags(@$item['id'], 'tags_group', 'news', $type, $config['website']['lang-default'])?>
                            </div>
                            <?php } ?>
                        </div>
                    </div>
                ...
                <div class="form-group form-group-gallery">
                    ...
                    <div class="jFiler-items my-jFiler-items jFiler-row">
                        <ul class="jFiler-items-list jFiler-items-grid row scroll-bar" id="jFilerSortable">
                            <?php foreach($gallery as $v) echo $func->galleryFiler($v['stt'],$v['id'],$v['photo'],$v['ten'.$config['website']['lang-default']],'news','col-xl-2 col-lg-3 col-md-3 col-sm-4 col-6'); ?>
                        </ul>
                    </div>
                </div>
                <?php } ?>
