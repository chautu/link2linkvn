<?php
    ...
    $linkDeletePhoto = "index.php?com=user&act=delete-photo-timeline-admin&p=".$curPage;
    $linkAddPhoto = "index.php?com=user&act=add-photo-timeline-admin&p=".$curPage;
    $linkEditPhoto = "index.php?com=user&act=edit-photo-timeline-admin&p=".$curPage;
?>
<!-- Content Header -->
    ...
    <div class="container-fluid">
        <div class="row">
            <ol class="breadcrumb float-sm-left">
                ...
                <li class="breadcrumb-item active"><a href="javascript:void(0);" onclick="history.back();" title="Thêm giao dịch">Cập nhật bài viết</a></li>
            </ol>
        </div>
    </div>
</section>
...
<section class="content">
    <form class="validation-form" novalidate method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
        ...
        <div class="card card-primary card-outline text-sm">
            ...
            <?php $user = $func->get_profile($_SESSION[$login_admin]['id'], "user"); ?>
        ...
        <div class="d-flex justify-content-end">
                <a class="btn btn-sm bg-gradient-primary text-white mr-3" href="<?=$linkAddPhoto?>&id-timeline=<?= $item['id'] ?>" title="Thêm mới"><i class="fas fa-plus mr-2"></i>Thêm mới</a>
                <a class="btn btn-sm bg-gradient-danger text-white" id="delete-all" data-url="<?=$linkDeletePhoto?>&id-timeline=<?= $item['id'] ?>" title="Xóa tất cả"><i class="far fa-trash-alt mr-2"></i>Xóa tất cả</a>
        ...
        <div class="card card-primary card-outline text-sm mt-4 mb-4">
            ...
            <div class="card-body table-responsive p-0">
                <table class="table table-hover">
                    ...
                    <?php $timeline_file = $func->get_rows("user_photo", "id_timeline",$item['id']); if(empty($timeline_file)) { ?>
                            ...
                            <?php foreach($timeline_file as $tl_file) { ?> 
                                <tr>
                                    ...
                                    <td class="align-middle text-center">
                                        <div class="custom-control custom-checkbox my-checkbox">
                                            <input type="checkbox" class="custom-control-input show-checkbox" id="show-checkbox-<?=$tl_file['id']?>" data-table="user_photo" data-id="<?=$tl_file['id']?>" data-loai="hienthi" <?=($tl_file['hienthi'])?'checked':''?>>
                                            <label for="show-checkbox-<?=$tl_file['id']?>" class="custom-control-label"></label>
                                        </div>
                                    </td>
                                    <td class="align-middle text-center text-md text-nowrap">
                                        <a class="text-primary mr-2" href="<?=$linkEditPhoto?>&id=<?=$tl_file['id']?>&id-timeline=<?= $item['id'] ?>" title="Chỉnh sửa"><i class="fas fa-edit"></i></a>
                                        <a class="text-danger" id="delete-item" data-url="<?=$linkDeletePhoto?>&id=<?=$tl_file['id']?>&id-timeline=<?= $item['id'] ?>" title="Xóa"><i class="fas fa-trash-alt"></i></a>
                                    </td>
                                </tr>
                            <?php } ?>
                        </tbody>
