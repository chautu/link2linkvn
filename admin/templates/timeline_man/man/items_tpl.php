<?php
    ...
    $linkAdd = "index.php?com=timeline&act=add_mem";
	$linkEdit = "index.php?com=timeline&act=edit_mem";
    $linkFilter = "index.php?com=timeline&act=man_mem";
        ...
        <div class="form-group col-xl-2 col-lg-3 col-md-4 col-sm-4 mb-2">
            <select id="id_member" name="id_member" onchange="onchange_category($(this), '<?= $linkFilter ?>')" class="form-control select2 filer-category">
                ...
                <?php foreach($member as $mem) { ?> 
                    <option <?= isset($_GET['id_member']) && $_GET['id_member'] == $mem['id'] ? "selected" : "" ?> value="<?= $mem['id'] ?>"><?= $mem['email'] ?></option>
                <?php } ?>
            </select>
        </div>
    </div>
                    ...
                    <tbody>
                        <?php for($i=0;$i<count($items);$i++) { ?>
                            <tr>
                                ...
                                <td class="align-middle">
                                    <a class="text-dark text-capitalize" href="<?=$linkUser?>&id=<?=$items[$i]['id_member']?>" title="<?= $func->get_profile($items[$i]['id_member'],'member')['email'] ?>"><?= $func->get_profile($items[$i]['id_member'],'member')['email'] ?></a>
                                </td>
