...
$linkFilter = "index.php?com=product&act=man&type=".$type;
...
<?php if(
 ...
 <div class="card-footer form-group-category text-sm bg-light row">
   <?php if(isset($config['product'][$type]['list']) && $config['product'][$type]['list'] == true) { ?>
    <div class="form-group col-xl-2 col-lg-3 col-md-4 col-sm-4 mb-2"><?=$func->get_link_category('product', 'list', $type, $config['website']['lang-default'])?></div>
  <?php } ?>
  <?php if(isset($config['product'][$type]['cat']) && $config['product'][$type]['cat'] == true) { ?>
    <div class="form-group col-xl-2 col-lg-3 col-md-4 col-sm-4 mb-2"><?=$func->get_link_category('product', 'cat', $type, $config['website']['lang-default'])?></div>
  <?php } ?>
  <?php if(isset($config['product'][$type]['item']) && $config['product'][$type]['item'] == true) { ?>
    <div class="form-group col-xl-2 col-lg-3 col-md-4 col-sm-4 mb-2"><?=$func->get_link_category('product', 'item', $type, $config['website']['lang-default'])?></div>
  <?php } ?>
  <?php if(isset($config['product'][$type]['sub']) && $config['product'][$type]['sub'] == true) { ?>
    <div class="form-group col-xl-2 col-lg-3 col-md-4 col-sm-4 mb-2"><?=$func->get_link_category('product', 'sub', $type, $config['website']['lang-default'])?></div>
  <?php } ?>
  <?php if(isset($config['product'][$type]['brand']) && $config['product'][$type]['brand'] == true) { ?>
    <div class="form-group col-xl-2 col-lg-3 col-md-4 col-sm-4 mb-2"><?=$func->get_link_category('product', 'brand', $type, $config['website']['lang-default'], false, 'Chọn hãng')?></div>
  <?php } ?>
</div>
<?php } ?>
    ...
    <?php } else { ?>
      <tbody>
        <?php for($i=0;$i<count($items);$i++) {
         ...
         if($items[$i]['id_street']) $linkID .= "&id_street=".$items[$i]['id_street']; 
         ?>
         <tr>
          ...
          <?php if(isset($config['product'][$type]['show_images']) && $config['product'][$type]['show_images'] == true) { ?>
            <td class="align-middle">
             <a href="<?=$linkEdit?><?=$linkID?>&id=<?=$items[$i]['id']?>" title="<?=$items[$i]['ten'.$config['website']['lang-default']]?>"><img class="rounded img-preview" onerror="src='assets/images/noimage.png'" src="<?=THUMBS?>/<?=$config['product'][$type]['thumb']?>/<?=UPLOAD_PRODUCT_L.$items[$i]['photo']?>" alt="<?=$items[$i]['ten'.$config['website']['lang-default']]?>"></a>
           </td>
         <?php } ?>
         <td class="align-middle">
          ...
          <div class="tool-action mt-2 w-clear">
           <?php if(isset($config['product'][$type]['view']) && $config['product'][$type]['view'] == true) { ?>
            <a class="text-primary mr-3" href="<?=$linkView?><?=$items[$i]['tenkhongdau'.$config['website']['lang-default']]?>" target="_blank" title="<?=$items[$i]['ten'.$config['website']['lang-default']]?>"><i class="far fa-eye mr-1"></i>View</a>
          <?php } ?>
          <a class="text-info mr-3" href="<?=$linkEdit?><?=$linkID?>&id=<?=$items[$i]['id']?>" title="<?=$items[$i]['ten'.$config['website']['lang-default']]?>"><i class="far fa-edit mr-1"></i>Edit</a>
         ...
         <?php } ?>
         <a class="text-danger" id="delete-item" data-url="<?=$linkDelete?><?=$linkID?>&id=<?=$items[$i]['id']?>" title="<?=$items[$i]['ten'.$config['website']['lang-default']]?>"><i class="far fa-trash-alt mr-1"></i>Delete</a>
       </div>
     </td>
     <?php if(isset($config['product'][$type]['gallery']) && count($config['product'][$type]['gallery']) > 0) { ?>
      <td class="align-middle">
