...
<section class="content">
	<form class="validation-form" novalidate method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
		...
		<?php if(isset($config['website']['debug-developer'])) { ?>
			<div class="card card-primary card-outline text-sm">
				...
				<div class="card-body">
					<div class="form-group">
						<?php if(isset($config['website']['debug-developer-mailhost']) && $config['website']['debug-developer-mailhost'] == true) { ?>
						...
						<?php }?>
						<div class="custom-control custom-radio d-inline-block mr-3 text-md">
							<input class="custom-control-input mailertype" type="radio" id="mailertype-gmail" name="data[options][mailertype]" <?=($options['mailertype']==2) ||!empty($options['mailertype']) ?"checked":""?> value="2">
					...
					<?php }?>
					<div class="gmail-email <?=($options['mailertype']==2) || !empty($options['mailertype']) ?'d-block':'d-none'?>">
						<div class="row">
							<div class="form-group col-md-4 col-sm-6">
								<label for="host_gmail">Host:</label>
								<input type="text" class="form-control" name="data[options][host_gmail]" id="host_gmail" placeholder="Host" value="<?=$options['host_gmail']?>" readonly>
							</div>
							<div class="form-group col-md-4 col-sm-6">
								<label for="port_gmail">Port:</label>
								<input type="text" class="form-control" name="data[options][port_gmail]" id="port_gmail" placeholder="Port" value="<?=$options['port_gmail']?>" readonly>
							</div>
							<div class="form-group col-md-4 col-sm-6">
								<label for="secure_gmail">Secure:</label>
								<select class="form-control" name="data[options][secure_gmail]" id="secure_gmail" readonly>
							...
							<div class="form-group col-md-4 col-sm-6">
								<label>Cách cấu hình:</label>
								<div style="line-height: 38px">
									<span class="text-danger">*</span> <a href="">Hướng dẫn lấy password cấu hình gmail</a>
								</div>
							</div>
						</div>
					</div>
				...
				<div class="card card-primary card-outline card-outline-tabs">
					<div class="card-header p-0 border-bottom-0">
						<ul class="nav nav-tabs" id="custom-tabs-three-tab-lang" role="tablist">
							<?php foreach($config['website']['lang'] as $k => $v) { ?>
								<li class="nav-item">
									<a class="nav-link <?=($k==$config['website']['lang-default'])?'active':''?>" id="tabs-lang" data-toggle="pill" href="#tabs-lang-<?=$k?>" role="tab" aria-controls="tabs-lang-<?=$k?>" aria-selected="true"><?=$v?></a>
								</li>
							<?php } ?>
						</ul>
					</div>
					<div class="card-body card-article">
						<div class="tab-content" id="custom-tabs-three-tabContent-lang">
							<?php foreach($config['website']['lang'] as $k => $v) { ?>
								<div class="tab-pane fade show <?=($k==$config['website']['lang-default'])?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
									<div class="form-group">
										<label for="ten<?=$k?>">Tiêu đề (<?=$k?>):</label>
										<input type="text" class="form-control for-seo" name="data[ten<?=$k?>]" id="ten<?=$k?>" placeholder="Tiêu đề (<?=$k?>)" value="<?=@$item['ten'.$k]?>" <?=($k==$config['website']['lang-default'])?'required':''?>>
									</div>
								</div>
							<?php } ?>
						</div>
				...
				<?php /* if(count($config['website']['lang']) > 1) { ?>
					<div class="form-group">
						...
						<div class="form-group">
							<?php foreach($config['website']['lang'] as $k => $v) { ?>
								<div class="custom-control custom-radio d-inline-block mr-3 text-md">
									<input class="custom-control-input" type="radio" id="lang_default-<?=$k?>" name="data[options][lang_default]" <?=($k==$config['website']['lang-default'] ? "checked" : ($k==$options['lang_default'])) ? "checked" : ""?> value="<?=$k?>">
				...
				<?php if(isset($config['setting']['toado_iframe']) && $config['setting']['toado_iframe'] == true) { ?>
					<div class="form-group">
						<label for="toado_iframe">
							...
							<a class="text-sm font-weight-normal ml-1" href="">(<span class="text-danger">*</span>Hướng dẫn lấy mã nhúng)</a>
						</label>
