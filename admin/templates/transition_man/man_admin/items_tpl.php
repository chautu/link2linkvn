<?php
    ...
    $linkAdd = "index.php?com=transition&act=add_admin";
	$linkEdit = "index.php?com=transition&act=edit_admin";
    $linkFilter = "index.php?com=transition&act=man_admin";

    $linkUser = "index.php?com=user&act=edit_admin";
?>
...
<section class="content">
    ...
    <div class="card-footer form-group-category text-sm bg-light row">
        ...
        <div class="form-group col-xl-2 col-lg-3 col-md-4 col-sm-4 mb-2">
            <select id="id_user" name="id_user" onchange="onchange_category($(this), '<?= $linkFilter ?>')" class="form-control select2 filer-category">
				<option value="" selected="" hidden="">Lọc username</option>
                <?php foreach($member as $mem) { ?> 
                    <option <?= isset($_GET['id_user']) && $_GET['id_user'] == $mem['id'] ? "selected" : "" ?> value="<?= $mem['id'] ?>"><?= $mem['username'] ?></option>
                <?php } ?>
            </select>
        </div>
    </div>
                    ...
                    <tbody>
                        <?php for($i=0;$i<count($items);$i++) { ?>
                            <tr>
                                ...
                                <td class="align-middle">
                                    <a class="text-dark text-capitalize" href="<?=$linkUser?>&id=<?=$items[$i]['id_user']?>" title="<?= $func->get_profile($items[$i]['id_user'],'user')['username'] ?>"><?= $func->get_profile($items[$i]['id_user'],'user')['username'] ?></a>
                                </td>
                                <td class="align-middle">
